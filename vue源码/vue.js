...
(function (global, factory) {
  /*
	模块导出: AMD   CMD  CommonJs  ES6 形式
	AMD: 异步加载（RequireJS实现）  模块定义：define
	CMD: 同步加载（SeaJs 实现）     模块定义：define
	CommonJs：浏览器不支持，nodeJs后端使用  模块导出 module.exports    exports.area
	ES6特性： export {}  export default {}   引入 import 
	*/
  ...
  /**
   * Check if a tag is a built-in tag. 检查是否是内置标签
   
   map:{
	   slot: true
	   component: true
   }
   return function (val) {return map[val]}   val是否是map的值
   */
  var isBuiltInTag = makeMap('slot,component', true);

  /**
   * Check if an attribute is a reserved attribute. 检查这些属性是否是保留的属性、
   map的可以为key,ref,slot,slot-scope,is
   return function (val) {return map[val]}   val是否是map的值
   */
   ...
   * Create a cached version of a pure function.
   创建一个函数缓存
   得到可以获取缓存中固定键的值的函数，值为undef时则是给缓存赋值
   */
  function cached(fn) {
   ...
   * Camelize a hyphen-delimited string.
   将一个str缓存到一个对象中，如果这个对象中存在这个obj[str]，则返回缓存中的obj[str]，
  如果没有，则赋值obj[str] = fun(str)（将str转化为小驼峰），并返回obj[str]
   
   */
  var camelizeRE = /-(\w)/g;
  var camelize = cached(function (str) {
    // str.replace第二个参数是函数时，该函数的第一个参数是匹配模式的字符串。
   ...
   * Merge an Array of Objects into a single Object.
   将一个数组中的值全部放到一个{}中，数组中的值为对象，则全部赋值到{}中
   */
   ...
   * Generate a string containing static keys from compiler modules.
   将一个模块数组每一项的staticKeys值放到一个数组中，将结果用','分割再返回
   */
   ...
   * if they are plain objects, do they have the same shape?
   判断两个值是否相等，数组每一项值相等，对象键值都对应，时间相等
   */
   ...
   * contain an object of the same shape), or -1 if it is not present.
   判断特定值与一个数组中的哪个值相等，相等则返回index，不相等返回-1
   */
   ...
   * Ensure a function is called only once.
   确保fn只被调用一次：
   使用： let onceFun = once(fn)
   onceFun('fsfs') onceFun('fsfs1') 不执行
   */
  function once(fn) {
    var called = false;
    return function () {
  ...
  var config = {
    /**
     * Option merge strategies（策略） (used in core/util/options)
     */
    // $flow-disable-line
  ...
  /*  */
  // 全局的警告函数，提示函数， 组件追踪函数（追踪到具体的一个组件），格式化组价名称
  // 先复制为一个空函数，后边进行重新赋值函数
  var warn = noop;
  var tip = noop;
  // 组件追踪函数（追踪到具体的一个组件名称）
...

  {
    ...
    // 将一个字符串中的首字母大写，如果有-_则将-_ 替换为空格，并将-_后的第一个首字母大写
    // fsfs-fsf => FsfsFsf
    ...
    // 格式化组件名称  eg: fd-fd => FdFd   fdfd => Fdfd
    formatComponentName = function (vm, includeFile) {
      ...
      // 子组件实例配置
      var options =
        typeof vm === 'function' && vm.cid != null ? vm.options
          : vm._isVue ? vm.$options || vm.constructor.options
          : vm;
    ...
    // 复制，不用for循环减少循环次数,提升性能，所用次数log2为底n的对数次 + 1
   ...
   */
  // 将array的一些方法复制到arrayMethods对象中，使用这个对象中的方法时会被监测
  ...
  */
  ...
  function defineReactive$$1(obj, key, val, customSetter, shallow) {
    ...
    // 返回obj对应的key在obj的描述，{value: 1, wirteAble: true, ...} 无则返回undefined
    ...
    // 对为观察的每一个值进行递归观察，最后返回观察的对象
    var childOb = !shallow && observe(val);
    // 重新定义obj中的key的get，set方法，可以再改变值，获取值时可以及时的通知相关地方，（双向绑定）
    Object.defineProperty(obj, key, {
      enumerable: true,
      configurable: true,
   ...
   * already exist.
   */
  // 在一个对象（target）中，查询一个属性（key），并重新赋值（val）
  // 重置属性值，执行dep.notify(),通知
   ...
   */
  // 将父组件与子组件的值进行合并的策略处理函数 {}
  // {data: fun, mounted: fun, propsData: fun。。。。}
  ...
  function mergeData(to, from) {
    ...
    for (var i = 0; i < keys.length; i++) {
      ...
      toVal = to[key];
      fromVal = from[key];
      // to中没有from的key，to中添加属性
      if (!hasOwn(to, key)) {
        // to中设置一个属性，这个属性就会执行一些列的监测操作，并执行to.dep.notify(),通知与to相关的值
        set(to, key, fromVal);
      } else if (
        toVal !== fromVal &&
        isPlainObject(toVal) &&
  ...
  function mergeHook(parentVal, childVal) {
    // 判断是不是有childVal，没有res = childVal，  有则返回childVal的数组形式
    var res = childVal?
        parentVal?
        parentVal.concat(childVal) : 
  ...
  // 策略对象添加hook strats = {data: fun, propsData: fun, beforeMount: fun, mounted: fun}
  // strats.mounted = mergeHook(parnetInstance, childInstance)
  ...
  // component , 指令directive  过滤器 filter
  ...
  function validateComponentName(name) {
    ...
    // isBuiltInTag 判断name是否是slot component, 判断定义的组件名称是不是html中的名称
  ...
  function validateProp(key, propOptions, propsData, vm) {
    ...
    // 自己定义的key肯定不是propsdata中的key
    var absent = !hasOwn(propsData, key);
    var value = propsData[key];
    // boolean casting  array  boolean
  ...
  // 判断value是否与 string number boolean是否相等
  ...
  {
    // 创建一个对象，对象的key是map函数的字符串参数，makeMap函数返回一个函数，目的查看返回函数的参数是不是在makeMap创建的对象内
